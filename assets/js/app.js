...
const TYPES = {
  ...
  RFCE: { code:"RFCE", name:"コンテクストペンギン", color:"#26a69a",
    ...
    anecdotes:["ワークショップや街歩きが好き", "課題の敷地分析がめちゃ丁寧", "「人がどう使うか」をずっと考えている", "コンセプトのキーワードがあたたかい", "模型に人添景置きがち", "講評会で「温かい空間ですね」と褒められる", "スタジオの空気がピリつくと静かに仲裁"],
    ...
    workNotes:["椅子の座り心地にうるさい", "模型に人添景を置きがち", "自分も住みたくなる空間をつくる"],
...
const HERO_CHAR_LAYOUT = [
  ...
  { top: 40, left: 7, size: 60, rotate: -6, delay: 1.3, dur: 3.8 },
...
function compatHtml(items) {
  return items.map(g => {
    ...
    const escapedBody = esc(body).replace(/\n/g, '<br>').replace(/解決策<br>/g, '<strong class="compat-solution">解決策</strong><br>');
    const bodyHtml = body ? `<p class="compat-body">${escapedBody}</p>` : '';
...
function renderShareImage(t, charImg, scores) {
  ...
  const cx = bx + bw - 400, cy = by + 120, cs = 360;
  ctx.save();
